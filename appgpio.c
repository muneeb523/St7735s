...
/**
 * Read a 32-bit register from a given physical address.
 * @param phys_addr Physical address of the register (e.g., 0x3033014C)
 * @return The 32-bit value read from the register, or UINT32_MAX on error
 */
uint32_t read_register(uint32_t phys_addr) {
    ...
    // Calculate the offset and read the register
    reg = (uint32_t *)(map_base + (phys_addr & PAGE_MASK));
    value = *reg;
...
/**
 * Write a 32-bit value to a given physical register address.
 * @param phys_addr Physical address of the register (e.g., 0x3033014C)
...
int write_register(uint32_t phys_addr, uint32_t value) {
    ...
    // Calculate the offset and write to the register
    reg = (uint32_t *)(map_base + (phys_addr & PAGE_MASK));
    *reg = value;
