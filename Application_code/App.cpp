  ...
  void Enter_Power_Mode()
{
    ...
    {
        const std::string unbind_path = "/sys/bus/sdio/drivers/wlan_sdio/unbind";
        const std::string bind_path   = "/sys/bus/sdio/drivers/wlan_sdio/bind";
        const std::string power_state_path = "/sys/power/state";
        const std::string device_id = "mmc0:0001:1";

        // Unbind the SDIO WiFi driver
        {
            std::ofstream unbind_stream(unbind_path);
            if (!unbind_stream.is_open())
            {
                std::cerr << "Error: Unable to open " << unbind_path << std::endl;
                return;
            }
            unbind_stream << device_id << std::endl;
        }

        // Suspend to RAM
        {
            std::ofstream power_state_stream(power_state_path);
            if (!power_state_stream.is_open())
            {
                std::cerr << "Error: Unable to open " << power_state_path << std::endl;
                return;
            }
            power_state_stream << "mem" << std: :endl;
        }

        // System resumes here after GPIO wakeup

        // Rebind the SDIO WiFi driver
        {
            std::ofstream bind_stream(bind_path);
            if (!bind_stream.is_open())
            {
                std::cerr << "Error: Unable to open " << bind_path << std::endl;
                return;
            }
            bind_stream << device_id << std::endl;
        }

        std::cout << "System resumed and WiFi re-bound successfully." << std::endl;
    }
}
